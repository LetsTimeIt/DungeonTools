...
local db
local tonumber, tinsert, pairs, ipairs, tostring, twipe, max, tremove, DrawLine =
    tonumber,
    table.insert,
    pairs,
    ipairs,
...
function MDT:DoFramesOverlap(frameA, frameB, offset)
    ...
    return ((frameALeft * sA) < (frameBRight * sB)) and ((frameBLeft * sB) < (frameARight * sA)) and
        ((frameABottom * sA) < (frameBTop * sB)) and
        ((frameBBottom * sB) < (frameATop * sA))
end
        ...
        for _, blip in pairs(blips) do
            if blip.clone.g == self.clone.g then
                blip.fontstring_Text1:SetText(
                    MDT:IsCurrentPresetTeeming() and blip.data.teemingCount or blip.data.count
                )
            ...
            self:SetScript(
                ...
                function()
                    ...
                    if x ~= nx or y ~= ny then
                        ...
                        if not isClose then
                            targetPull = nil
                            MDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(
                                self,
                                false,
                                IsControlKeyDown(),
                                tempPulls,
...
local function setUpMouseHandlersAwakened(self, clone, scale, riftOffsets)
                ...
                function()
                    ...
                    if x ~= self.adjustedX or y ~= self.adjustedY then
                        ...
                        self.animatedLine =
                            MDT:ShowAnimatedLine(
                            MDT.main_frame.mapPanelFrame,
                            connectedDoor or self.spireFrame,
                            self,
                            nil,
                            nil,
                            nil,
                        ...
                        self:ClearAllPoints()
                        self:SetPoint("CENTER", MDT.main_frame.mapPanelTile1, "TOPLEFT", x * MDTscale, y * MDTscale)
                    end
                end
            )
        end
    ...
    self:SetScript(
        ...
        function()
            ...
            self:SetPoint(
                "CENTER",
                MDT.main_frame.mapPanelTile1,
                "TOPLEFT",
                self.adjustedX * scale,
                self.adjustedY * scale
            )
...
function DungeonToolsEnemyMixin:DisplayPatrol(shown)
    ...
    for _, line in pairs(patrolLines) do
        line:Hide()
    end
    if not shown then
        return
    ...
    if self.clone.patrol then
        ...
        for patrolIdx, waypoint in ipairs(self.clone.patrol) do
            patrolPoints[patrolIdx] =
                patrolPoints[patrolIdx] or
            ...
            patrolPoints[patrolIdx]:SetPoint(
                "CENTER",
                MDT.main_frame.mapPanelTile1,
                "TOPLEFT",
                waypoint.x * scale,
...
function MDT:DisplayBlipTooltip(blip, shown)
    ...
    if blip.data.reaping and db.MDI.enabled and preset.mdi.beguiling == 13 then
        local reapingIcon =
            CreateTextureMarkup(
            MDT.reapingStatic[tostring(blip.data.reaping)].iconTexture,
            32,
            32,
            16,
            16,
        ...
        reapingText =
            L["Reaping"] .. ": " .. reapingIcon .. " " .. MDT.reapingStatic[tostring(blip.data.reaping)].name .. "\n"
    end
    if reapingText then
        text = text .. "\n" .. reapingText
    end
...
local function blipDevModeSetup(blip)
    ...
    blip:SetScript(
        "OnDragStart",
        function()
            if not db.devModeBlipsMovable then
                return
            end
            blip:StartMoving()
        end
    ...
    blip:SetScript(
        "OnMouseWheel",
        function(self, delta)
            if not db.devModeBlipsScrollable then
                return
            end
            ...
            if blip.clone.g then
                blip.fontstring_Text1:SetTextColor(unpack(groupColors[blip.clone.g % 5 + 1]))
            end
        end
    )
    blip.fontstring_Text1:Show()
...
function DungeonToolsEnemyMixin:SetUp(data, clone)
    ...
    self.normalScale =
        data.scale * (data.isBoss and 1.7 or 1) * (MDT.scaleMultiplier[db.currentDungeonIdx] or 1) * scale
    ...
    if not data.corrupted then
        for _, v in pairs(blips) do
            ...
            if ((clone.x - v.clone.x) ^ 2 + (clone.y - v.clone.y) ^ 2 < 81) and MDT:DoFramesOverlap(self, v, 5) then
                raise = max(raise, v:GetFrameLevel() + 1)
            end
        end
    end
    ...
    if data.isBoss then
        self.texture_Dragon:Show()
    else
        self.texture_Dragon:Hide()
    end
    ...
    if data.corrupted then
        --hide by default, DungeonEnemies_UpdateSeasonalAffix handles showing
    ...
    if emissaryIds[self.data.id] then
        self:Hide()
    end --hide beguiling emissaries by default
    tinsert(blips, self)
...
function MDT:FindPullOfBlip(blip)
    ...
    local pulls = currentPreset.value.pulls or {}

    for pullIdx, pull in ipairs(pulls) do
        if pull[blip.enemyIdx] then
            for _, cloneIdx in ipairs(pull[blip.enemyIdx]) do
...
function MDT:GetBlip(enemyIdx, cloneIdx, presetArg)
    for _, blip in pairs(blips) do
...
function MDT:DungeonEnemies_AddOrRemoveBlipToCurrentPull(blip, add, ignoreGrouped, pulls, pull, ignoreUpdates)
    ...
    pull = pull or currentPreset.value.currentPull
    pulls = pulls or currentPreset.value.pulls or {}
...
function MDT:DungeonEnemies_GetPullColor(pull, pulls)
    ...
    pulls = pulls or preset.value.pulls
    r, g, b = MDT:HexToRGB(pulls[pull]["color"])
    if not r then
        r, g, b = MDT:HexToRGB(db.defaultColor)
        ...
        if pull[enemyIdx] then
            if cloneIdx then
                for _, pullCloneIndex in pairs(pull[enemyIdx]) do
                    if pullCloneIndex == cloneIdx then
                        return true
                    end
                end
            else
                for _, _ in pairs(pull[enemyIdx]) do
...
function MDT:GetEnemyForces(npcId)
    if countCache[npcId] then
        return countCache[npcId].count, countCache[npcId].maxCountNormal, countCache[npcId].maxCountTeeming, countCache[
            npcId
        ].teemingCount
    else
                ...
                for _, enemy in pairs(data) do
                    if enemy.id == npcId then
                        ...
                        if enemy.corrupted then
                            ...
                            for _, corruptedEnemy in pairs(data) do
                                if corruptedEnemy.id == npcId then
                                    return corruptedEnemy.count, self.dungeonTotalCount[dungeonIdx].normal, self.dungeonTotalCount[
                                        dungeonIdx
                                    ].teeming, corruptedEnemy.teemingCount
                                end
                            end
                        else
                            countCache[npcId] = {
            ...
            for _, enemy in pairs(enemyData) do
                ...
                if newData[enemy.id] and (enemy.id ~= 155432 and enemy.id ~= 139110) then
                    if newData[enemy.id].count then
                        ...
                        if
                            newData[enemy.id].count ~= newData[enemy.id].teeming_count and
                                (newData[enemy.id].count ~= enemy.count or
                                    newData[enemy.id].teeming_count ~= enemy.teemingCount)
                         then
                            shouldPrintDungeonName = printDungeonName(shouldPrintDungeonName, dungeonIdx)
                            print(
                                "TEEMING ",
                                enemy.name,
                                enemy.id,
                                newData[enemy.id].count,
                                "||",
                                newData[enemy.id].teeming_count
                            )
                            enemy.count = newData[enemy.id].count
            ...
            for _, enemy in pairs(enemyData) do
                if not enemy.displayId then
                    if shouldAddDungeonText then
                        output =
                            output .. "Dungeon(name='" .. MDT:GetDungeonName(idx) .. "', idx=" .. idx .. ", npcIds=["
                        shouldAddDungeonText = false
                    end
                    output = output .. enemy.id .. ", "
                end
            end
            if not shouldAddDungeonText then
                output = output .. "]),\n"
            end
        end
    end
