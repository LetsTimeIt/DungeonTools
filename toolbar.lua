...
function MDT:initToolbar(frame)
    ...
    frame.toolbar.toggleButton:SetScript(
        "OnClick",
        function()
    ...
    colorPicker:SetCallback(
        "OnValueConfirmed",
        function(widget, callbackName, r, g, b, a)
    ...
    minus:SetCallback(
        ...
        function(widget, callbackName)
            ...
            if db.toolbar.brushSize < 1 then
                db.toolbar.brushSize = 1
            end
    ...
    sizeIndicator.editbox:SetScript(
        "OnMouseWheel",
        function(self, delta)
    ...
    pencil:SetCallback(
        ...
        function(widget, callbackName)
            if currentTool == "pencil" then
                ...
                MDT:UpdateSelectedToolbarTool("pencil")
            end
        end
    )
    ...
    line:SetCallback(
        ...
        function(widget, callbackName)
            if currentTool == "line" then
                ...
                MDT:UpdateSelectedToolbarTool("line")
            end
        end
    )
    ...
    arrow:SetCallback(
        "OnClick",
        function(widget, callbackName)
            if currentTool == "arrow" then
                MDT:UpdateSelectedToolbarTool()
            else
                MDT:UpdateSelectedToolbarTool("arrow")
            end
        end
    )
    ...
    note:SetCallback(
        "OnClick",
        function(widget, callbackName)
            if currentTool == "note" then
                MDT:UpdateSelectedToolbarTool()
            else
                MDT:UpdateSelectedToolbarTool("note")
            end
        end
    )
    ...
    mover:SetCallback(
        ...
        function(widget, callbackName)
            if currentTool == "mover" then
                ...
                MDT:UpdateSelectedToolbarTool("mover")
            end
        end
    )
    ...
    eraser:SetCallback(
        ...
        function(widget, callbackName)
            if currentTool == "eraser" then
                ...
                MDT:UpdateSelectedToolbarTool("eraser")
            end
        end
    )
        ...
        function(widget, callbackName)
            ...
            self:OpenConfirmationFrame(
                ...
                function()
                    ...
                    if self.liveSessionActive then
                        self:LiveSession_SendCommand("deletePresetObjects")
                    end
                end
            )
        end
        ...
        function(self, tick)
            if MouseIsOver(MDTScrollFrame) and not MouseIsOver(MDTToolbarFrame) then
                ...
                if tool == "eraser" then
                    ...
                    frame.brushPreview.tex:SetVertexColor(
                        db.toolbar.color.r,
                        db.toolbar.color.g,
                        db.toolbar.color.b,
                        db.toolbar.color.a
...
function MDT:UpdateSelectedToolbarTool(widgetName)
    ...
    if not widgetName or (not toolbarTools[widgetName]) then
        if toolbar.highlight then
            toolbar.highlight:Hide()
        end
        MDT:RestoreScrollframeScripts()
        MDT:DisableBrushPreview()
        if drawingActive then
            if currentTool == "pencil" then
                MDT:StopPencilDrawing()
            end
            if currentTool == "arrow" then
                MDT:StopArrowDrawing()
            end
            if currentTool == "line" then
                MDT:StopLineDrawing()
            end
            if currentTool == "mover" then
                MDT:StopMovingDrawing()
            end
            if currentTool == "eraser" then
                MDT:StopEraserDrawing()
            end
        end
        currentTool = nil
    ...
    frame.scrollFrame:SetScript(
        ...
        function(self, button)
            if button == "LeftButton" then
                if currentTool == "pencil" then
                    MDT:StartPencilDrawing()
                end
                if currentTool == "arrow" then
                    MDT:StartArrowDrawing()
                end
                if currentTool == "line" then
                    MDT:StartLineDrawing()
                end
                if currentTool == "mover" then
                    MDT:StartMovingObject()
                end
                if currentTool == "eraser" then
                    MDT:StartEraserDrawing()
                end
            end
...
function MDT:StartArrowDrawing()
    ...
    nobj = {
        d = {
            db.toolbar.brushSize,
            1,
            MDT:GetCurrentSubLevel(),
            true,
        ...
        function(self, tick)
            ...
            if x ~= startx and y ~= starty then
                DrawLine(
                    ...
                    x,
                    y,
                    (db.toolbar.brushSize * 0.3) * scale,
                    1,
                    "TOPLEFT"
                )
        ...
        function(self, tick)
            ...
            if endx ~= startx and endy ~= starty then
                DrawLine(
                    ...
                    endx,
                    endy,
                    (db.toolbar.brushSize * 0.3) * 1.1 * scale,
                    1.00,
                    "TOPLEFT"
                )
                line:SetDrawLayer(objectDrawLayer, drawLayer)
...
function MDT:StopMovingObject()
    ...
    if objectIndex then
        ...
        if self.liveSessionActive then
            self:LiveSession_SendObjectOffsets(objectIndex, originalX - newX, originalY - newY)
        end
    end
    objectIndex = nil
    drawingActive = false
                                ...
                                if v.points then
                                    if
                                        (v.points[1] == x1 and v.points[2] == y1) or
                                            (v.points[1] == x2 and v.points[2] == y2)
                                     then
                                        v:Hide()
                                    end
                                end
...
function MDT:StopEraserDrawing()
    ...
    frame.toolbar:SetScript("OnUpdate", nil)
    if self.liveSessionActive then
        self:LiveSession_SendUpdatedObjects(changedObjects)
    end
    MDT:DrawAllPresetObjects()
    drawingActive = false
end
...
local function updateNoteObjText(text, note)
    ...
    if MDT.liveSessionActive then
        MDT:LiveSession_SendNoteCommand("text", note.objectIndex, text)
    end
end
local function deleteNoteObj(note)
    ...
    if MDT.liveSessionActive then
        MDT:LiveSession_SendNoteCommand("delete", note.objectIndex, "0")
    end
    MDT:DrawAllPresetObjects()
end
...
function MDT:DrawNote(x, y, text, objIndex)
    ...
    function note:OpenEditBox()
        if not noteEditbox then
            noteEditbox = makeNoteEditbox()
        end
