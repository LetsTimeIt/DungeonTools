...
local function convex_hull(pts)
    local lower_left = 1
    for i = 2, #pts do
        if is_lower_left(pts[i], pts[lower_left]) then
            lower_left = i
        end
    end
    ...
    repeat
        ...
        final = 1
        for j = 2, #pts do
            if lower_left == final or is_left_of(pts[lower_left], pts[final], pts[j]) then
                final = j
            end
        end
        lower_left = final
...
--[[ Unused Functions, commented out to prevent warnings
    ...
    if hull and hull[#hull] and #hull > 2 then
        ...
        for i = 1, #hull do
            ...
            MDT:DrawHullLine(
                ...
                b[1],
                b[2],
                3 * (MDT.scaleMultiplier[MDT:GetDB().currentDungeonIdx] or 1),
                pullColor,
                true,
                "ARTWORK",
                -8,
                ...
                if MDT:IsCloneIncluded(enemyIdx, cloneIdx) then
                    for _, blip in pairs(blips) do
                        if (blip.enemyIdx == enemyIdx) and (blip.cloneIdx == cloneIdx) then
                            local _, _, _, endX, endY = blip:GetPoint()
...
function MDT:FindClosestPull(x, y)
    ...
    local centerIndex
    for k, centerLoc in pairs(centers) do
        local squaredDist = (x - centerLoc[1]) ^ 2 + (y - centerLoc[2]) ^ 2
