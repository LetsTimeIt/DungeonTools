...
function MDT:LiveSession_NotifyEnabled()
    ...
    if not lastNotify or lastNotify < now - 0.2 then
        lastNotify = now
        local distribution = self:IsPlayerInGroup()
        if (not distribution) or (not self.liveSessionActive) then
            return
        end
            ...
            function()
                if self.liveSessionActiveSessions[1][1] ~= fullNamePlayer then
                    ...
                    if not self.main_frame.LoadingSpinner then
                        self.main_frame.LoadingSpinner =
                            CreateFrame("Button", "MDTLoadingSpinner", self.main_frame, "LoadingSpinnerTemplate")
    ...
    if self:GetCurrentPreset().uid == self.livePresetUID then
        ...
        if distribution then
            ...
            commsObject:SendCommMessage(
                self.liveSessionPrefixes.ping,
                x * (1 / scale) .. ":" .. y * (1 / scale) .. ":" .. sublevel,
                distribution,
                nil,
                "ALERT"
            )
...
function MDT:LiveSession_SendObjectOffsets(objIdx, x, y)
    if self:GetCurrentPreset().uid == self.livePresetUID then
        ...
        if distribution then
            commsObject:SendCommMessage(
                self.liveSessionPrefixes.objOff,
                objIdx .. ":" .. x .. ":" .. y,
                distribution,
                nil,
                "ALERT"
