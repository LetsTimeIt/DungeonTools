...
local function tshow(t, name, indent)
    ...
    local function isemptytable(checkedTable)
        return next(checkedTable) == nil
    end
    ...
    local function addtocart(value, nameArg, indentArg, saved, field)
        indentArg = indentArg or ""
        saved = saved or {}
        field = field or nameArg

        cart = cart .. indentArg .. field
        ...
        if type(value) ~= "table" then
            ...
            if saved[value] then
                ...
                autoref = autoref .. nameArg .. " = " .. saved[value] .. ";\n"
            else
                saved[value] = nameArg
                ...
                if isemptytable(value) then
                    ...
                    for k, v in pairs(value) do
                        k = basicSerialize(k)
                        local fname = sformat("%s[%s]", nameArg, k)
                        field = sformat("[%s]", k)
                        -- three spaces between levels
                        addtocart(v, fname, indentArg .. "   ", saved, field)
                    end
                    cart = cart .. indentArg .. "};\n"
                end
            end
...
function MDT:CreateDevPanel(frame)
        ...
        local buttons = {
            ...
            [4] = {
                ...
                func = function()
                    ...
                    tinsert(
                        pois,
                        {x = posx, y = posy, template = "MapLinkPinTemplate", type = "generalNote", text = noteText}
                    )
                    MDT:POI_UpdateAll()
                end
            },
            ...
            [6] = {
                ...
                func = function()
                    ...
                    tinsert(
                        pois,
                        {x = posx, y = posy, template = "MapLinkPinTemplate", type = "mechagonBot", botIndex = botIndex}
                    )
                    MDT:POI_UpdateAll()
                end
            },
    ...
    local function DrawGroup2(container)
        ...
        button2:SetCallback(
            ...
            function()
                ...
                if currentBlip then
                    --encounterID
                    local encounterID, _, _, _, _ = EJ_GetCreatureInfo(1)
                    if not encounterID then
                        print(
                            "MDT: Error - Make sure to open Encounter Journal and navigate to the boss you want to add!"
                        )
                        return
                    end
                    for i = 1, 10000 do
    ...
    function MDT:UpdateMap(...)
        originalFunc(...)
        local selectedTab
        for _, v in pairs(devPanel.tabs) do
...
function MDT:AddPatrolWaypointAtCursorPosition()
    ...
    if currentBlip then
        ...
        for _, waypoint in pairs(patrolBlips) do
        ...
        print(
            string.format(
                "MDT: Created Waypoint %d of %s %d at %d,%d",
                1,
                data.name,
                #cloneData.patrol,
                cursorx,
