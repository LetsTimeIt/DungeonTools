...
local tinsert, SetPortraitToTexture, SetPortraitTextureFromCreatureDisplayID, MouseIsOver =
    table.insert,
...
local function GetDropTarget()
    ...
    if scrollFrame.frame:IsMouseOver(1, -1, -dragdrop_overlap, dragdrop_overlap) then
        ...
        repeat
            ...
            if id and button then
                ...
                pos =
                    (button.frame:IsMouseOver(2, offset, -dragdrop_overlap, dragdrop_overlap) and "TOP") or
                    (button.frame:IsMouseOver(-offset, -1, -dragdrop_overlap, dragdrop_overlap) and "BOTTOM")
            end
        until not id or pos
    ...
    -- Top Graceful Area
    if
        scrollFrame.frame:IsMouseOver(100, scroll_frame_height + 1, -dragdrop_overlap, dragdrop_overlap) and
            scroll_value < scroll_value_min
     then
    ...
    if
        scrollFrame.frame:IsMouseOver(-(scroll_frame_height + 1), -100, -dragdrop_overlap, dragdrop_overlap) and
...
local methods = {
    ...
    ["Initialize"] = function(self)
        ...
        function self.callbacks.OnClickNormal(_, mouseButton)
            if not MouseIsOver(MDT.main_frame.sidePanel.pullButtonsScrollFrame.frame) then
                return
            end
                ...
                if (mouseButton == "RightButton") then
                    ...
                    if #MDT:GetSelection() > 1 then
                        L_EasyMenu(
                            ...
                            MDT.main_frame.sidePanel.optionsDropDown,
                            "cursor",
                            0,
                            -15,
                            "MENU"
        ...
        function self.callbacks.OnEnter()
            ...
            --progressbar
            if MDT.ProgressBarResetTimer then
                MDT.ProgressBarResetTimer:Cancel()
            end
        ...
        function self.callbacks.OnKeyDown(self, key)
            --[[ Commenting out to prevent warning as this does nothing.
            if (key == "ESCAPE") then
            --
            end
            --]]
    ...
    ["GetScrollSpeed"] = function(self, frame, speedList)
        for _, entry in ipairs(speedList) do
                    ...
                    if
                     ...
                     then
                        ...
                        if self.top_hover > scroll_hover.timeout then
                            ...
                            local scroll_pulls =
                                MDT.U.lerp(
                                scroll_hover.pulls_per_second.min,
                                scroll_hover.pulls_per_second.max,
                                scroll_speed
                            )
                            local scroll_pixel = scroll_pulls * self.frame:GetHeight()
                            local scroll_amount =
                                MDT:GetScrollingAmount(scrollFrame, scroll_pixel) * scroll_hover.timeout
                    ...
                    elseif
                     ...
                     then
                        ...
                        if self.bottom_hover > scroll_hover.timeout then
                            ...
                            if
                                scrollFrame.localstatus.scrollvalue == oldvalue and newvalue >= 0 and newvalue <= 1000 and
                                    oldvalue < 985
                             then
    ...
    ["Drop"] = function(self)
        local insertID, _, pos = GetDropTarget()
    ...
    ["Disable"] = function(self)
        ...
        for _, v in pairs(self.enemyPortraits) do
            v:Hide()
            v.overlay:Hide()
            v.fontString:Hide()
        end
    ...
    ["SetNPCData"] = function(self, enemyTable)
        ...
        for _, data in ipairs(enemyTable) do
            idx = idx + 1
            ...
            if data.displayId then
                ...
                SetPortraitToTexture(
                    self.enemyPortraits[idx],
                    "Interface\\Icons\\achievement_boss_hellfire_mannorothreanimated"
                )
            end
